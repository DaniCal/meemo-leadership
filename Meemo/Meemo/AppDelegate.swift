...
//        self.window = MBFingerTipWindow(frame: UIScreen.main.bounds)
    ...
    func loadImages(){
        for lecture in lectures {
            ...
            loadPortrait(lecture: lecture)
        }
    }
    
    func loadPortrait(lecture: Lecture){
        Alamofire.request(lecture.portraitURL).response { response in
            debugPrint(response)
            if let data = response.data {
                lecture.portraitImage = UIImage(data: data)!
            }else{
            }
            
            self.loadedPortraits = self.loadedPortraits + 1
            if(self.loadedPortraits == self.lectures.count && self.loadedImages == self.lectures.count){
                let launchedBefore = UserDefaults.standard.bool(forKey: "launchedBefore")
                if(launchedBefore){
    ...
    func loadImage(lecture: Lecture){
        Alamofire.request(lecture.imageURL).response { response in
            ...
            if(self.loadedImages == self.lectures.count && self.loadedPortraits == self.lectures.count){
                let launchedBefore = UserDefaults.standard.bool(forKey: "launchedBefore")
                if(launchedBefore){
                    self.showHomeViewController()
                }
